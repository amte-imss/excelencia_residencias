...
class Gestion_revision_model extends MY_Model {
    ...
    public function get_sn_comite($param = []) {
        try {
            ...
            $this->db->select(array('s.id_solicitud', 's.matricula', 'i.nombre', 'i.apellido_paterno', 'i.apellido_materno', 'del.nombre as delegacion', 's.fecha','(select count(*) from excelencia.revision rev where rev.id_solicitud=s.id_solicitud) as total'));
